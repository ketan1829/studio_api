    ...
    try {
      ...
      if (userType === "NUMBER") {
        ...
        if (!TestNumbers.includes(phoneNumber)){
            const smsResponse = await axios.get(`https://www.fast2sms.com/dev/bulkV2?authorization=${process.env.FAST2SMS_AUTH_KEY}&variables_values=${token}&route=otp&numbers=${phoneNumber}`);
        ...
        }else if (TestNumbers.includes(phoneNumber)){
...
// NR
// exports.updateUserProfileUrl = async (req,res,next)=>{
//     await User.updateProfileUrl()
